...
/****************************************************
This library has been modified to use Hardware Serial Boards that have more than one
hardware serial, such as arduino Mega 2560. the fingerprint Sensor has to be connected
on the port (Rx-n, Tx-n) corresponding to the port defined in the sketch.
For example, if mySerial is defined as Serial1, the Adafruit fingerprint sensor is
connected as follows:

GREEN Wire --> RX1 (Digital Pin 19)
WHITE Wire --> TX1 (Digital Pin 18)

Modified by Maosa Samson
*****************************************************/
#include "Adafruit_Fingerprint_MEGA.h"

Adafruit_Fingerprint_MEGA::Adafruit_Fingerprint_MEGA(HardwareSerial *hs) {
  thePassword = 0;
...
void Adafruit_Fingerprint_MEGA::begin(uint16_t baudrate) {
  mySerial->begin(baudrate);
}

boolean Adafruit_Fingerprint_MEGA::verifyPassword(void) {
...
uint8_t Adafruit_Fingerprint_MEGA::getImage(void) {
...
uint8_t Adafruit_Fingerprint_MEGA::image2Tz(uint8_t slot) {
...
uint8_t Adafruit_Fingerprint_MEGA::createModel(void) {
...
uint8_t Adafruit_Fingerprint_MEGA::storeModel(uint16_t id) {
...
uint8_t Adafruit_Fingerprint_MEGA::emptyDatabase(void) {
...
uint8_t Adafruit_Fingerprint_MEGA::fingerFastSearch(void) {
  fingerID = 0xFFFF;
  confidence = 0xFFFF;
...
uint8_t Adafruit_Fingerprint_MEGA::getTemplateCount(void) {
...
void Adafruit_Fingerprint_MEGA::writePacket(uint32_t addr, uint8_t packettype, 
...
uint8_t Adafruit_Fingerprint_MEGA::getReply(uint8_t packet[], uint16_t timeout) {
